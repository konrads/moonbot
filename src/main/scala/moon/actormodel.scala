...
object TradeLifecycle extends Enumeration {
  ...
  val Issuing, Amending, Cancelling = Value
}

sealed trait ActorCtx
case class InitCtx(ledger: Ledger) extends ActorCtx
case class IdleCtx(ledger: Ledger) extends ActorCtx
case class OpenPositionCtx(ledger: Ledger, orderID: String=null, orderPrice: BigDecimal=null, lifecycle: TradeLifecycle.Value=TradeLifecycle.Issuing) extends ActorCtx
