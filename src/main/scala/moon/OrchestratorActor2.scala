...
object OrchestratorActor2 {
  ...
  case class Instrument(gauges: Map[String, Any], now: Option[Long]) extends SideEffect
  ...
  def asDsl(strategy: Strategy,
            ...
            useTrailingStoploss: Boolean = false)(implicit log: org.slf4j.Logger): (Ctx, ActorEvent) => (Ctx, Option[SideEffect]) = {
    // open (aka init)
    ...
    def tick(ctx: Ctx, event: ActorEvent)(implicit log: org.slf4j.Logger): (Ctx, Option[SideEffect]) = (ctx, event) match {
      ...
      case (_, SendMetrics(nowMs)) =>
        if (ctx.ledger.isMinimallyFilled) {
          ...
          val instrument = Instrument(ledger2.ledgerMetrics.metrics, nowMs)
          (ctx.withLedger(ledger2), Some(instrument))
        } else
          (ctx, None)
  ...
  def asLiveBehavior(behaviorDsl: (Ctx, ActorEvent) => (Ctx, Option[SideEffect]), restGateway: IRestGateway, metrics: Option[Metrics]=None, flushSessionOnRestart: Boolean=true)(implicit execCtx: ExecutionContext): Behavior[ActorEvent] = {
    Behaviors.withTimers { timers =>
      ...
      Behaviors.setup { actorCtx =>
        ...
        implicit val log: org.slf4j.Logger = actorCtx.log
        def loop(ctx: Ctx): Behavior[ActorEvent] =
          Behaviors.receiveMessage { event =>
            val (ctx2, effect) = behaviorDsl(ctx, event)
            effect.foreach {
              case Instrument(gauges, now) =>
  ...
  def asPaperBehavior(behaviorDsl: (Ctx, ActorEvent) => (Ctx, Option[SideEffect]), metrics: Option[Metrics]=None): Behavior[ActorEvent] = {
    Behaviors.withTimers { timers =>
      timers.startTimerAtFixedRate(SendMetrics(None), 1.minute)
