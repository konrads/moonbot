...
object Orchestrator {
  def asDsl(strategy: Strategy,
    ...
    def openPositionOrder(l: Ledger, symbol: String): Either[String, (OpenInitOrder, Tier)] = {
    ...
    def tick(ctx: Ctx, event: ActorEvent, log: org.slf4j.Logger, symbol: String): (Ctx, Option[SideEffect]) = (ctx, event) match {
        ...
        if (ledger2.isMinimallyFilled) {
          ...
          openPositionOrder(ledger2, symbol) match {
            case Right((effect, tier)) =>
              log.info(s"$symbol Init: starting afresh with $dir order: ${effect.clOrdID}, ${effect.qty} @ ${effect.price.get} in tier: $tier")
      ...
      case (IdleCtx(ledger), WsEvent(wsData)) =>
        ...
        openPositionOrder(ledger2, symbol) match {
          case Right((effect, tier)) =>
            log.info(s"$symbol Idle: starting afresh with $dir order: ${effect.clOrdID}, ${effect.qty} @ ${effect.price.get} in tier: $tier")
            (OpenPositionCtx(ledger = ledger2, clOrdID = effect.clOrdID, targetPrice = effect.price.get, lifecycle = IssuingNew), Some(effect))
          case Left(reason) =>
      ...
      case (ctx2@OpenPositionCtx(clOrdID, ledger, targetPrice, lifecycle), event@(WsEvent(_) | RestEvent(Success(_)))) =>
        ...
        (ordStatusOpt, clOrdIDMatch) match {
          ...
          case (Some(PostOnlyFailure), true) =>
            openPositionOrder(ledger2, symbol) match {
              case Right((effect, tier)) =>
                log.info(s"$symbol Open $dir: PostOnlyFailure for orderID: ${orderOpt.get.fullOrdID}, re-issuing order: clOrdID: ${effect.clOrdID}, ${effect.qty} @ ${effect.price.get} in tier: $tier")
        ...
        lifecycle match {
          case IssuingNew =>
            openPositionOrder(ledger, symbol) match {
              case Right((effect, tier)) =>
                log.info(s"$symbol Open: re-issuing $dir order: ${effect.clOrdID}, ${effect.qty} @ ${effect.price.get} due to error in tier: $tier", e)
      ...
      case (ctx2@ClosePositionCtx(openClOrdID, _, openQty, clOrdID, ledger, lifecycle), event@(WsEvent(_) | RestEvent(Success(_)))) =>
        ...
        (orderOpt, ordStatusOpt, clOrdIDMatch, isRestReply) match {
          ...
          case (Some(ord), Some(PostOnlyFailure), true, true) =>
            ...
            log.warn(s"$symbol Close $dir: re-issuing order dur to PostOnlyFailure, clOrdID: ${effect.takeProfitClOrdID} @ ${effect.takeProfitLimit}, qty: ${effect.qty}")
          ...
          case (Some(ord), _, true, _) =>
            if (lifecycle != Awaiting)
              log.info(s"$symbol Close $dir: got $event update to order ${ord.fullOrdID}, setting lifecycle $lifecycle -> $Awaiting")
      ...
      case (ctx2@ClosePositionCtx(_, _, _, _, ledger, lifecycle), On30s(_)) =>
        openPositionOrder(ledger, symbol) match {
          case Right((effect, tier)) =>
            if (lifecycle == Awaiting) {
              log.info(s"$symbol Close: issuing new tier $dir order: ${effect.clOrdID}, ${effect.qty} @ ${effect.price.get} in tier: $tier")
              (OpenPositionCtx(ledger = ledger, clOrdID = effect.clOrdID, targetPrice = effect.price.get, lifecycle = IssuingNew), Some(effect))
            } else {
              if (log.isDebugEnabled) log.debug(s"$symbol Close $dir: didn't issue new tier $dir order: ${effect.clOrdID}, ${effect.qty} @ ${effect.price.get} in tier: $tier, due to lifecycle: $lifecycle")
              (ctx2, None)
            }
          case Left(reason) =>
