...
class LedgerSpec extends FlatSpec with Matchers with Inside {
  ...
  it should "work for incremental and full pandl" in {
    ...
    metrics5 shouldBe Some(LedgerMetrics(Map("data.price" -> BigDecimal(55), "data.pandl" -> BigDecimal("-0.2313269230769230769230769230769231"), "data.pandlDelta" -> BigDecimal("-0.2313269230769230769230769230769231"), "data.sentimentScore" -> BigDecimal(0), "data.myTradesCnt" -> 3, "data.volume" -> 0.0, "data.tickDirScore" -> 0.0), parseDateTime("2010-01-05T00:00:00.000Z"), BigDecimal("-0.2313269230769230769230769230769231")))  // no buy/sell as yet
    ...
    metrics7 shouldBe Some(LedgerMetrics(Map("data.price" -> BigDecimal(55), "data.pandl" -> BigDecimal("-0.2328269230769230769230769230769231"), "data.pandlDelta" -> BigDecimal("-0.00150"), "data.sentimentScore" -> BigDecimal(0), "data.myTradesCnt" -> 4, "data.volume" -> 0.0, "data.tickDirScore" -> 0.0), parseDateTime("2010-01-07T00:00:00.000Z"), BigDecimal("-0.2328269230769230769230769230769231")))  // no buy/sell as yet
  }
