...
class LedgerSpec extends FlatSpec with Matchers with Inside {
  ...
  it should "work for incremental and full pandl" in {
    ...
    val l2 = l.withMetrics()
    val metrics2 = l2.ledgerMetrics
    metrics2 shouldBe None  // no buy/sell as yet

    // add sell and buy, recalculate metrics, expect the last buy to be ignored
    val l3 = buildLedger(l2,
      // o2 - buy filled
    ...
    l3.ledgerOrders.toSeq.map(_.orderID) shouldBe Seq("o6", "o5", "o4", "o3", "o2", "o1")

    // add orderbook data
    val l4 = l3.record(OrderBook("blah", "update", Seq(OrderBookData("xbtusd", "t0001", asks=Seq(Seq(10, 20), Seq(20, 30)), bids=Seq(Seq(100, 200), Seq(200, 300))))))

    val l5 = l4.withMetrics()
    val metrics5 = l5.ledgerMetrics
    metrics5 shouldBe Some(LedgerMetrics(Map("price" -> BigDecimal(55), "pandl" -> BigDecimal(-30.0425), "pandlDelta" -> BigDecimal(-30.0425)), Some("t5"), BigDecimal(-30.0425)))  // no buy/sell as yet
    ...
    metrics7 shouldBe Some(LedgerMetrics(Map("price" -> BigDecimal(55), "pandl" -> BigDecimal(-.15), "pandlDelta" -> BigDecimal(29.89250)), Some("t7"), BigDecimal(-.15)))  // no buy/sell as yet
  }
