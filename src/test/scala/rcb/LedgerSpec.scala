...
class LedgerSpec extends FlatSpec with Matchers with Inside {
  "Ledger" should "work with WS and REST orders" in {
    val ws1 = UpsertOrder(Some("insert"), data=Seq(
      OrderData(orderID="o1", price=Some(1), orderQty=1, side=OrderSide.Buy, timestamp="2010-01-01", ordStatus=Some(OrderStatus.New)),
      OrderData(orderID="o2", price=Some(2), orderQty=2, side=OrderSide.Buy, timestamp="2010-01-02", ordStatus=Some(OrderStatus.New)),
      OrderData(orderID="o3", price=Some(3), orderQty=3, side=OrderSide.Buy, timestamp="2010-01-03", ordStatus=Some(OrderStatus.New)),
    ))
    val ws2 = UpsertOrder(Some("update"), data=Seq(
      OrderData(orderID="o2", price=Some(2), orderQty=2, side=OrderSide.Buy, timestamp="2010-01-02", ordStatus=Some(OrderStatus.Canceled), text=Some("had execInst of ParticipateDoNotInitiate")),
    ))
